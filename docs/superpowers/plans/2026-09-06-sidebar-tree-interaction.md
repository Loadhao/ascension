...
**Goal:** 侧边栏目录树支持点击分组只切自身开合（不联动后代；整棵子树靠 `.sl-tree-toggle` / 工具条）、顶部「全部展开/全部收起」工具条、每分组子树开合按钮，并用 localStorage 跨标签页/重启持久化展开状态。
...
按设计规格「验证」节执行五项：点 Java 只露出基础/中级/高级折叠项（不铺开内部分类；整棵子树靠 `.sl-tree-toggle`）；工具条全开/全收（收起保留当前链路）；hover 子树按钮仅作用该组、触屏模拟常显；换页/新标签页/重启浏览器状态保持；移动抽屉可用。`pnpm build` 通过，预览无控制台报错。
...
- 点击任意分组标题只切换该组自身，不联动后代；分类（叶分组）行为与现状一致；整棵子树开合靠 `.sl-tree-toggle` / 工具条
