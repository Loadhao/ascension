...
| 项 | 值 |
|---|---|
| 状态 | r2 已确认（r1 2026-09-06；r2 按用户反馈修订：全收无例外、新增文档定位、按钮轻量化） |
...
| 与原生恢复的竞争 | 原生 sessionStorage 恢复的异步 toggle 曾冲掉已存意图（实测发现） | 子树联动只由用户点击触发，`toggle` 仅记账；r2 实测通过 |
| 后台标签页 rAF 暂停 | rAF 去抖在后台/嵌入视图静默失效（实测发现） | 去抖与联动延迟一律用 `setTimeout(0)` |
