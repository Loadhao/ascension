...
| 14. `scripts/evolution-candidates.mjs` 的车道游标数组与配方 §1 车道表不一致（**第 194 轮实测**）：脚本第 48 行 `const lane = ['', 'A 新章节', 'B 影像资产', 'C 题库', 'B 影像资产', 'D 体检与工具'][next % 5]` 仍是影像阶段「B 占两格」的旧表，而配方 §1 已于 2026-09-25 把其中一格还给 A（`1、4 → A`）。后果只在 `n mod 5 == 4` 时显形——第 194 轮开工实跑该命令打印「194 mod 5 = 4 → **车道 B 影像资产**」，按配方应为 **A**，本轮按配方执行并如实登记差异（1、2、3、0 四格两侧一致，所以此前从未暴露）。改法：把该数组 index 4 由 `'B 影像资产'` 改为 `'A 新章节'`（1 文件 1 行），落地后用 194 复算应打印 A、并回归 191/192/193/195 四格打印不变 | 修问题 | 3 | 1.0 | 1 | 3.0 | 待办（D 队列，第 194 轮实测入队；不改配方只改脚本） |
