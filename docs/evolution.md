...
### 第 95 轮（2026-09-08，第三十二次启动，内容补充模式）

- 选择：时区处理（case-studies 第 23 篇）——grep 确认时区/UTC 4 篇分散（语言层）无架构层纪律篇；国际化后端必踩坑题。
- 交付：`23-timezone.md`（存储/传输/展示三层分离图、MySQL DATETIME vs TIMESTAMP 与 PG timestamptz 行为差异、夏令时坑、服务器与前端时区不可信、差 8 小时四步排查清单）+ 侧边栏注册 + 图谱 tz 节点与 1 条边。
- 验证：五连验证全绿（build 627 页）；提交 0d6fef4 已推送（本篇纯表格）。
- 结论：补功能。
- 下一轮入口：候选池——①场景题：消息幂等细化/数据归档；②ai 线：Agent 记忆分层细化；③linux 线歇。每轮开工先同步+定界+体检基线。
...
- 验证：五连验证全绿；提交 0d6fef4 已推送（本篇纯表格）。**过程小错即改：level 字段笔误（basic vs intermediate 目录）体检拦截后修复**——与第 67/90 轮同款，第三次发生，见经验段。
- 结论：补功能。
- 下一轮入口：数据归档（grep 冷热分离仅 1 句确认）。

### 第 96 轮（2026-09-08，第三十三次启动）

- 选择：数据归档：冷热分离（case-studies 第 24 篇）——归档 18 篇分散、冷热分离仅设计框架篇一句；与 logrotate/Online DDL 互链的运维场景。
- 交付：`24-archive.md`（在线表膨胀代价、冷热分离架构图与查询路由、分批删除/先归档后删除/幂等纪律、与分库分表正交、分区表关系）+ 侧边栏注册 + 图谱 archive 节点与 1 条边。
- 验证：五连验证全绿（build 628 页）；提交 58bd0b1 已推送（含时区篇 level 修复）。
- 结论：补功能。
- 新证据与经验：①case-studies 系列到 24 篇；②**level 笔误第三次发生（67/90/95 轮各一次）**：根因是"写完即提交"流程中 frontmatter 无自动检查——已由体检第 7 项拦截，但根因改善建议：新篇 frontmatter 写完后核对 level 与目录等级一致（写入检查清单）；③本启动三轮状态文件同步执行到位（94 轮教训的纠正落地）。
- 下一轮入口：候选池——①场景题：秒杀变种同构避、数据订阅（CDC/Debezium）grep 排查；②ai 线：Agent 工具集设计篇候选（工具粒度/描述质量/错误返回）；③linux 线歇。每轮开工先同步+定界+体检基线。
