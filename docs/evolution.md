...
- 选择：**阻塞项置顶**——基线体检失败 1 项：并行会话新推的 React/Vue/操作系统理论等 5 个提交中，`linux/basic/theory/03-io-multiplexing.mdx` 的 frontmatter 写了 `level: intermediate` 与 basic 目录不一致（同分类 01/02 均为 basic，笔误无疑）。
- 交付：1 行修正 level: intermediate → basic。
- 验证：体检恢复 8 项全绿；build 599 页通过；提交 d245f32 已推送。
- 结论：修问题（并行会话笔误，修复方式唯一无争议，如实上报）。
- 新证据与经验：①并行会话新增 **React/Vue 两个前端方向**（各 3 篇起步）+ Dubbo 分类 + 操作系统理论分类——站点从 547 涨至 599 页；②React/Vue 为并行会话新方向（刚规划），本轮避让其主人继续。
- 下一轮入口：接口幂等（transaction 分类勘察）。

### 第 68 轮（2026-09-08，第二十四次启动）

- 选择：接口幂等篇（transaction/03）——勘察 transaction 分类（分布式事务/分布式 ID 两篇）后成稿；但**提交前体检暴露图谱节点 id 重复**：并行会话同期已提交 `coordination/02-idempotency`（138 行/六方案/5 条边/更完整）——撞题。
- 交付：①**撤回撞题篇**：删除自己的未提交文件 transaction/03-idempotency.md（零风险），还原侧边栏与图谱注册；②修复并行会话 Dubbo 篇的 mermaid 语法错误（边标签以数字开头需引号，会导致整页正文静默丢失的老问题）。
- 验证：撤回后体检 9 项全绿（含图谱结构项抓到 id 重复）、mermaid 语法全绿；build 599 页；提交 851e260 已推送。
- 结论：修问题（撞题撤回 + 语法修复）。
- 新证据与经验：①**撞题新形态**：并行会话在"另一个分类"写了同主题（coordination vs transaction），按分类勘察会漏——撞题预警要查图谱节点与全站标题，不能只查目标分类；②体检第 9 项（图谱结构）在真实撞题中发挥价值；③mermaid 边标签以数字开头是语法雷区（需引号）。
- 下一轮入口：多模态 LLM（grep 确认 AI 方向零覆盖）。

### 第 69 轮（2026-09-08，第二十四次启动）

- 选择：多模态 LLM（ai/intermediate/llm 第 8 篇）——grep 确认多模态/VLM 全站零覆盖；图片→视觉 token 的机制与计费，AI 应用上升期高频。
- 交付：`08-multimodal.md`（patch 切块 → ViT 编码 → 投影对齐流水线图、视觉 token 计费与分辨率旋钮、截图排查/票据/图表问答应用、OCR 边界）+ 侧边栏注册 + 图谱 multimodal 节点与 1 条边。
- 验证：五连验证全绿（build 600 页/对比度 318 页——并行会话新增图表一并确认）；提交 6a33cac 已推送。
- 结论：补功能。
- 新证据与经验：①站点突破 600 页；②ai/intermediate/llm 分类到 8 篇（…→量化→多模态）；③本轮 2 修复 + 1 内容——并行会话新方向上线期，修复轮占比自然升高，属正常节奏。
- 下一轮入口：候选池——①React/Vue 新方向满 3 篇但方向主人刚起步，持续避让观察；②场景题：订单履约（发货/物流）grep 排查；③ai/intermediate/agent 第 12 篇（结构化输出与约束解码）。每轮开工先同步+定界+体检基线。

### 第 70 轮（2026-09-08，第二十五次启动，内容补充模式）

- 选择：结构化输出（ai/intermediate/agent 第 12 篇）——grep 确认约束解码/受限解码零覆盖（结构化提示仅在提示工程篇，解析容错/JSON mode/受限解码四层防线无专篇）；AI 应用落地必考。
- 交付：`12-structured-output.md`（四层防线流程图（提示→解析容错→JSON mode→受限解码）、JSON mode"语法合法≠语义正确"边界、schema 版本兼容与评测回归）+ 侧边栏注册 + 图谱 structout 节点与 2 条边。
- 验证：五连验证全绿（build 601 页/对比度 319 页）；提交 af355a5 已推送。
- 结论：补功能。
- 下一轮入口：评论系统（grep 三关键词零覆盖确认）。

### 第 71 轮（2026-09-08，第二十五次启动）

- 选择：评论系统（case-studies 第 15 篇）——评论系统/热评/两级评论三关键词全站零覆盖；UGC 场景与点赞/敏感词篇组合。
- 交付：`15-comments.md`（两级拍平建模（不做无限树的理由）、根评论游标分页+回复折叠、热度分时间衰减算法、标记删与计数对账、先审后发风控分级）+ 侧边栏注册 + 图谱 comments 节点与 1 条边。
- 验证：五连验证全绿（build 602 页）；提交 798a186 已推送。
- 结论：补功能。
- 下一轮入口：埋点上报（grep 仅 10 篇分散提及，无场景专篇）。

### 第 72 轮（2026-09-08，第二十五次启动）

- 选择：埋点与数据上报（case-studies 第 16 篇）——反向高并发写入系统，与订单系统"必达"约束完全相反的架构对照；与可观测性三支柱（指标来源）互链。
- 交付：`16-analytics.md`（本地批量合并与 sendBeacon、失败容忍分级（埋点丢 1% 无感）、MQ 削峰 + eventId 幂等、Schema 宽表 JSON 演进、OLAP 分析链路图）+ 侧边栏注册 + 图谱 analytics 节点与 1 条边。
- 验证：五连验证全绿（build 603 页/对比度 320 页）；提交 2e702cf 已推送。
- 结论：补功能。
- 新证据与经验：①case-studies 系列到 16 篇，UGC 链（评论/点赞/签到）与数据链（埋点/可观测性）成型；②"容忍度决定架构"是埋点篇的核心洞察——同构系统的架构差异来自数据价值分级。
- 下一轮入口：候选池——①场景题：Excel 导入导出（异步任务/失败回执）grep 排查；②ai/intermediate/agent 第 13 篇（多模态 Agent/语音输入）；③linux/basic/commands 第 4 篇（用户与进程快捷查询，注意与 intermediate 02 进程管理避让）。每轮开工先同步+定界+体检基线。

### 第 73 轮（2026-09-08，第二十六次启动，内容补充模式）

- 选择：Excel 导入导出（case-studies 第 17 篇）——grep 确认无专篇（仅 python csv 格式篇与速答提及）；中后台必考场景。
- 交付：`17-excel.md`（同步三宗罪（OOM/超时/不可重试）、流式导出与 SXSSF、异步任务中心状态机+OSS 治理、导入两级校验+错误行回执+upsert 幂等）+ 侧边栏注册 + 图谱 excel 节点与 1 条边。
- 验证：五连验证全绿（build 604 页/对比度 321 页）；提交 c6215b5 已推送。
- 结论：补功能。
- 下一轮入口：审批流（中后台核心，状态机复杂度天花板）。

### 第 74 轮（2026-09-08，第二十六次启动）

- 选择：审批流（case-studies 第 18 篇）——多级/条件路由/驳回/加签，状态机复杂度天花板；朴素 if 崩溃点与引擎分界是考点主体。
- 交付：`18-approval-flow.md`（单链状态机数据模型（节点实例表）、引擎分界经验线、驳回三种语义与回退目标配置、动态加签、审批人抽象为岗位、超时自动升级）+ 侧边栏注册 + 图谱 approval 节点与 1 条边。
- 验证：五连验证全绿（build 605 页）；提交 1ae6f1d 已推送（本篇纯表格无 mermaid）。
- 结论：补功能。
- 下一轮入口：数据权限（grep 确认行级过滤无专篇）。

### 第 75 轮（2026-09-08，第二十六次启动）

- 选择：数据权限：行级过滤（case-studies 第 19 篇）——中后台必考，功能权限与数据权限分层、RBAC 数据范围四档、拦截器 SQL 注入、部门树 path 法、索引性能影响。
- 交付：`19-data-permission.md` + 侧边栏注册 + 图谱 dataperm 节点与 1 条边。
- 验证：五连验证全绿（build 606 页）；提交 03ab1b3 已推送（本篇纯表格无 mermaid）。
- 结论：补功能。
- 新证据与经验：①case-studies 系列到 19 篇，中后台链（审批流/数据权限/Excel 任务中心）成型；②与 linux/permission 篇形成"权限"纵深（系统层→应用层），跨方向同主题各管一层是好的组织方式。
- 下一轮入口：候选池——①场景题：库存对账/资金对账体系（红包/库存/支付对账的体系化汇总，查重叠）；②ai/intermediate/agent 第 13 篇（多模态 Agent 延伸）；③linux/intermediate/system 第 7 篇（crontab 定时任务与 systemd timer 对比，03 篇只提了一嘴）。每轮开工先同步+定界+体检基线。

### 第 76 轮（2026-09-08，第二十七次启动，内容补充模式）

- 选择：对账体系（case-studies 第 20 篇）——"对账"24 篇分散提及（红包/库存/支付/优惠券各篇的收尾句），体系化汇总收口篇；资金系统"确定性防线"的原理层。
- 交付：`20-reconciliation.md`（两套独立记录互证的本质、三层核对与准实时/T+1 分层防线图、排序归并/哈希分桶算法、差错单闭环四步、"对账任务也要被监控"递归）+ 侧边栏注册 + 图谱 recon 节点与 1 条边。
- 验证：五连验证全绿（build 607 页/对比度 323 页）；提交 2a1642d 已推送。
- 结论：补功能。
- 下一轮入口：linux 定时任务（systemd 篇 timer 只提一嘴，grep crontab linux 内 2 篇顺带）。

### 第 77 轮（2026-09-08，第二十七次启动）

- 选择：定时任务：crontab 与 systemd timer（linux/intermediate/system 第 7 篇）——运维基本功 + 分布式防重复执行的入口题。
- 交付：`07-cron-timer.md`（五字段与三坑（环境变量/%转义/输出重定向）、systemd timer 对比表与 Persistent 补跑、分布式防重复三思路（单机/分布式锁/调度平台）、"定时任务必须幂等"前提）+ 侧边栏注册 + 图谱 cronsched 节点与 1 条边。
- 验证：五连验证全绿（build 608 页）；提交 c72186e 已推送（本篇纯表格）。
- 结论：补功能。
- 下一轮入口：ai Embedding（RAG 两篇的底层，grep 确认无原理篇）。

### 第 78 轮（2026-09-08，第二十七次启动）

- 选择：Embedding：语义的向量表示（ai/basic/foundation 第 5 篇）——对比学习/相似度度量/MTEB 选型/微调边界；RAG 两篇的底层基础，AI 应用岗必问。
- 交付：`05-embedding.md`（对比学习与难负样本、三种度量表与归一化前提、MTEB 分榜选型、双塔 vs 拼接追问、向量库维度成本）+ 侧边栏注册 + 图谱 emb 节点与 1 条边（"向量检索的底座"→rag）。
- 验证：五连验证全绿（build 609 页）；提交 3479b35 已推送。
- 结论：补功能。
- 新证据与经验：①ai/basic/foundation 4→5 篇（ML→DL→Transformer→LoRA→Embedding），AI 基础线完整覆盖"训练-微调-表示"；②对账体系篇把 24 篇分散的"对账兜底"收口——**跨篇收口型选题**（同主题散落多篇时做体系化汇总）是新的选题类型，适合在系列成熟后使用。
- 下一轮入口：候选池——①场景题 grep 继续（活动页防刷/排队系统/秒杀变种）；②ai/intermediate/agent 第 13 篇（多模态 Agent/语音）；③linux/intermediate/system 已 7 篇，转 linux/basic/commands 第 4 篇（进程/用户快捷查询）。每轮开工先同步+定界+体检基线。

### 第 79 轮（2026-09-08，第二十八次启动，内容补充模式）

- 选择：排队系统（case-studies 第 21 篇）——grep 确认"排队系统/排队号"零覆盖；洪峰削峰的通用组件，与限流的"等待承诺"区别是考点主体。
- 交付：`21-queue.md`（发号+分桶队列+匀速消费架构图、进度反馈"求在动不求精确"、过号清理与防重复占号、VIP 分层的合法插队）+ 侧边栏注册 + 图谱 queuesys 节点与 1 条边（"削峰显式化"←秒杀）。
- 验证：五连验证全绿（build 610 页/对比度 324 页）；提交 65f14f0 已推送。
- 结论：补功能。
- 下一轮入口：语音交互（grep 仅 1 篇顺带，语音 Agent 上升期）。

### 第 80 轮（2026-09-08，第二十八次启动）

- 选择：语音交互：ASR 与 TTS 流水线（ai/intermediate/llm 第 9 篇）——多模态篇的姊妹篇（语音模态），语音 Agent 上升期高频；延迟账与打断是考点主体。
- 交付：`09-voice.md`（三段流水线延迟账、VAD 静音检测与 barge-in 打断、ASR 热词机制、流式 TTS 首包、三段流水线 vs 端到端的权衡）+ 侧边栏注册 + 图谱 voice 节点与 1 条边。
- 验证：五连验证全绿（build 611 页）；提交 8b2d070 已推送。
- 结论：补功能。
- 下一轮入口：LLM 网关（grep 三个关键词全站零覆盖确认）。

### 第 81 轮（2026-09-08，第二十八次启动）

- 选择：LLM 网关：多模型路由与统一管控（ai/intermediate/llm 第 10 篇）——grep（LLM 网关/模型路由/多模型）全站零覆盖；企业 AI 应用基础设施层，多模型时代的标配。
- 交付：`10-gateway.md`（直接调 API 五问题、网关五职责（统一接口/路由/配额/审计/密钥托管）、路由三策略（成本/能力/可用性）、流式透传实现要点、网关单点的降级）+ 侧边栏注册 + 图谱 gw 节点与 1 条边。
- 验证：五连验证全绿（build 612 页）；提交 c1a051e 已推送（本篇纯表格）。
- 结论：补功能。
- 新证据与经验：①ai/intermediate/llm 分类到 **10 篇**（本质→提示→选型→参数→成本→部署→量化→多模态→语音→网关），AI 应用主线从原理到基础设施全闭环；②场景题池（排队）与 AI 线（语音/网关）交替推进保持选题新鲜度。
- 下一轮入口：候选池——①ai/intermediate/agent 第 13 篇候选（多模态 Agent：视觉操作与语音 Agent 整合）；②linux/basic/commands 第 4 篇（进程/用户快捷查询，与 intermediate 02 避让）；③场景题 grep（IM 的群管理/消息漫游细化）。每轮开工先同步+定界+体检基线。

### 第 82 轮（2026-09-08，第二十九次启动，内容补充模式）

- 选择：视觉 Agent：看屏幕操作的计算机使用（ai/intermediate/agent 第 13 篇）——grep（computer use/视觉操作/RPA）零覆盖；多模态 LLM + Agent Loop 的整合形态，AI 前沿高频。
- 交付：`13-computer-use.md`（截图→理解→操作→再截图循环图、结构化动作输出（受限解码刚需）、RPA"泛化换精度"对比表、坐标/状态/成本三难点、护栏强制适用（白名单/确认/审计/沙箱））+ 侧边栏注册 + 图谱 compuse 节点与 2 条边（multimodal"看屏操作"、structout"动作格式约束"）。
- 验证：五连验证全绿（build 613 页/对比度 327 页）；提交 6e98ccd 已推送。
- 结论：补功能。
- 下一轮入口：Feature Flag（grep 三关键词仅 1 篇顺带确认）。

### 第 83 轮（2026-09-08，第二十九次启动）

- 选择：功能开关 Feature Flag（case-studies 第 22 篇）——全站仅注解地图顺带提及；与灰度发布篇互补（部署放量 vs 代码级开关）。
- 交付：`22-feature-flag.md`（四类开关生命周期表（发布开关=技术债制造机）、评估架构（平台+推送+内存缓存）、kill switch 分钟级 KPI、组合状态爆炸治理）+ 侧边栏注册 + 图谱 fflag 节点与 1 条边（gray→fflag"代码级开关"）。
- 验证：五连验证全绿（build 614 页）；提交 0d2b3cc 已推送。
- 结论：补功能。
- 下一轮入口：SSH 访问安全（linux 内 grep 三关键词零覆盖确认）。

### 第 84 轮（2026-09-08，第二十九次启动）

- 选择：SSH 密钥与远程访问安全（linux/basic/permission 第 3 篇）——SSH 三主题（密钥/加固/堡垒机）全站零覆盖；permission 线认证延伸，运维安全必考。
- 交付：`03-ssh-access.md`（挑战签名认证流程图（不传私钥）、加固四件套（关密码/禁 root/白名单/fail2ban）、跳板机堡垒机审计价值、agent 转发风险、known_hosts 防中间人）+ 侧边栏注册 + 图谱 ssh 节点与 1 条边。
- 验证：五连验证全绿（build 615 页/对比度 329 页）；提交 b193d93 已推送。
- 结论：补功能。
- 新证据与经验：①linux/basic/permission 3 篇成对（用户权限/属性能力/SSH），linux 方向 13 篇；②并行会话题库突破 500 题并持续深化（第二十一轮），内容-题库双轮驱动格局稳固；③视觉 Agent 篇把多模态/结构化输出/护栏三篇串成一体——**篇间串联写作**（新篇引用既有篇作基础）提升系列连贯性。
- 下一轮入口：候选池——①ai/basic/agent 或 intermediate/agent 间歇（两分类已厚，优先级降）；②linux/intermediate/system 第 8 篇（SSH 隧道/端口转发，与 SSH 篇衔接）；③场景题 grep（分布式文件同步/消息幂等细化）。每轮开工先同步+定界+体检基线。

### 第 85 轮（2026-09-08，第三十次启动，内容补充模式）

- 选择：SSH 隧道与端口转发（linux/intermediate/system 第 8 篇）——linux 方向 grep 隧道/端口转发零覆盖；SSH 篇续章，三种转发方向记忆是考点主体。
- 交付：`08-ssh-tunnel.md`（-L/-R/-D 三转发方向记忆口诀与对比图、跳板访问内网/反向演示/动态代理三场景、安全边界（ AllowTcpForwarding/审计）、autossh 保活、与 VPN 边界）+ 侧边栏注册 + 图谱 tunnel 节点与 1 条边。
- 验证：五连验证全绿（build 616 页/对比度 330 页）；提交 030091f 已推送。
- 结论：补功能。
- 下一轮入口：Agent 可观测性（grep 确认 trace/token 追踪无专篇）。

### 第 86 轮（2026-09-08，第三十次启动）

- 选择：Agent 可观测性（ai/intermediate/agent 第 14 篇）——三支柱分散提及无专篇；Agent 非确定性输出的监控维度（token/步数/工具成功率/trace 穿透）是护栏+评估之外的独立运行时维度。
- 交付：`14-agent-observability.md`（三支柱映射表、生命体征四信号表（token 突增/工具失败率/完成率/步数分布）、trace 穿透多模型调用链、坏 case 回流评测闭环图、分级留痕成本控制）+ 侧边栏注册 + 图谱 agentobs 节点与 1 条边。
- 验证：五连验证全绿（build 617 页）；提交 ffb67b5 已推送（本篇纯表格）。**过程小错即改：编辑 ai.json 产生双逗号导致构建失败——JSON 校验即时抓住，node 修复后全绿。**
- 结论：补功能。
- 下一轮入口：GPU 算力基础（Tensor Core 零覆盖、显存带宽/混合精度各仅一提）。

### 第 87 轮（2026-09-08，第三十次启动）

- 选择：GPU 与算力（ai/basic/foundation 第 6 篇）——vLLM（显存带宽瓶颈）/量化（显存账）/LoRA（显存节省）三篇共同的"为什么"底座；Tensor Core 零覆盖。
- 交付：`06-gpu.md`（矩阵乘与万核并行、算力/显存容量/显存带宽三概念图（推理是 memory-bound）、混合精度 BF16、训练 vs 推理负载差异表、买卡排序）+ 侧边栏注册 + 图谱 gpu 节点与 1 条边（"显存账的底层"→lora）。
- 验证：五连验证全绿（build 618 页/对比度 332 页）；提交 7cd1a76 已推送。
- 结论：补功能。
- 新证据与经验：①ai/basic/foundation 5→6 篇（…→Embedding→GPU），AI 基础线补齐算力底座；②**"底层为什么"选题法**：系列多篇共用一个未解释的底层概念时，写一篇底座篇让三篇的"为什么"同时有着落——GPU 篇一鱼三吃（vLLM/量化/LoRA）。
- 下一轮入口：候选池——①场景题 grep（排队变种/大促全链路压测）；②ai/basic/foundation 第 7 篇候选（多卡并行：数据/张量/流水线并行——GPU 篇追问埋点）；③linux 基础三分类均成对，歇一轮。每轮开工先同步+定界+体检基线。

### 第 88 轮（2026-09-08，第三十一次启动，内容补充模式）

- 选择：LLM 应用架构总装（ai/intermediate/llm 第 11 篇）——**收口型选题**：压测专篇勘察退场（performance/03 已有）后，发现 llm 分类 10 篇零件无总装图；"设计一个 ChatGPT 类应用"是 AI 面试大题。
- 交付：`11-architecture.md`（端到端总装图（网关/会话服务/RAG/模型层/流式/评测）、对话请求的一生五步、可靠性与成本两层防线、多模态扩展）+ 侧边栏注册 + 图谱 arch 节点与 1 条边。
- 验证：五连验证全绿（build 619 页/对比度 333 页）；提交 7764b5f 已推送。
- 结论：补功能。
- 下一轮入口：mysql 连接池（grep 确认 HikariCP 原理无专篇，28 篇分散提及）。

### 第 89 轮（2026-09-08，第三十一次启动）

- 选择：数据库连接池 HikariCP（mysql 新建 connection-pool 分类第 1 篇）——HikariCP 原理 28 篇分散提及无专篇；"池大小怎么算"是后端高频。
- 交付：三件套——`01-hikaricp.md`（连接成本账、**池大小公式（核数×2+盘数）不是越大越好**、泄漏与池内死锁两大事故、HikariCP 快在哪（ConcurrentBag/字节码/FastList））+ 分类页 + 侧边栏"连接池"分组 + 图谱 connpool 节点与 1 条边。
- 验证：五连验证全绿（build 621 页）；提交 eba4fb1 已推送（4 文件）。
- 结论：补功能。
- 下一轮入口：ai 多卡并行（GPU 篇追问埋点）。

### 第 90 轮（2026-09-08，第三十一次启动）

- 选择：多卡并行：数据/张量/流水线与 ZeRO（ai/basic/foundation 第 7 篇）——GPU 篇追问"为什么多卡"（显存墙+时间墙）的展开；训练岗高频、foundation 6→7 篇。
- 交付：`07-multi-gpu.md`（DP+AllReduce 与 ZeRO 三级消冗余、TP 层内切同机 NVLink、PP 流水线气泡与微批、3D 并行布局（TP 同机/PP 跨机/DP 外层）、推理多卡简化为 TP）+ 侧边栏注册 + 图谱 multigpu 节点与 1 条边。
- 验证：五连验证全绿（build 622 页）；提交 cac0d70 已推送（本篇纯表格）。
- 结论：补功能。
- 新证据与经验：①**收口型选题第二例**验证（对账篇之后）：LLM 应用架构总装篇把 10 篇零件装配成总装图——系列成熟后收口与铺零件交替；②mysql 方向本轮首次由本线新建分类（此前 mysql 内容归并行会话，quiz 仍归其管）。
- 下一轮入口：候选池——①场景题 grep：秒杀变种同构避、找非同构新场景（抽奖已写/排行榜避 redis）；②ai/basic/agent 间歇；③mysql/connection-pool 第二篇候选（事务与连接交互或多数据源）。每轮开工先同步+定界+体检基线。

### 第 91 轮（2026-09-08，第三十一次启动，内容补充模式）

- 选择：读写分离：连接路由与主从延迟（mysql/connection-pool 第 2 篇）——主从复制篇的第一应用，路由实现与主从延迟坑无专篇。
- 交付：`02-read-write-split.md`（写主读从架构图、SDK vs Proxy 路由对比表、ThreadLocal 路由键跨线程丢失坑、主从延迟分级解决表（默认容忍+关键读走主+位点等待））+ 侧边栏注册 + 图谱 rwsplit 节点与 2 条边。
- 验证：五连验证全绿（build 623 页/对比度 336 页）；提交 ab2f60a 已推送。
- 结论：补功能。
- 下一轮入口：企业知识库（grep 确认文档问答零覆盖）。

### 第 92 轮（2026-09-08，第三十一次启动）

- 选择：企业知识库：RAG 落地全流程（ai/intermediate/agent 第 15 篇）——知识库/文档问答 grep 零覆盖；RAG 技术两篇的场景化总装。
- 交付：`15-knowledge-base.md`（端到端架构图（文档管道/权限过滤/评测运营）、**检索层强制权限过滤**而非生成层自觉、增量同步、黄金问答集三指标、坏 case 运营闭环）+ 侧边栏注册 + 图谱 kb 节点与 1 条边。
- 验证：五连验证全绿（build 624 页）；提交 4842764 已推送。
- 结论：补功能。
- 下一轮入口：Tokenizer（llm 篇 token 概念深化，grep 确认无原理篇）。

### 第 93 轮（2026-09-08，第三十一次启动）

- 选择：Tokenizer：模型眼里的文字（ai/basic/foundation 第 8 篇）——BPE 直觉、字符级任务翻车根源、词表权衡；llm 篇 token 概念的原理层，AI 应用岗上升期考题。
- 交付：`08-tokenizer.md`（三个著名翻车根源表（数 r/算术/中文成本）、词表大小权衡、领域词扩充、应用影响三处）+ 侧边栏注册 + 图谱 tokenizer 节点与 1 条边；**顺带修复**：ai.json 的 transformer→lora 重复边（历轮编辑残留），并给 consistency-verify 第 9 项增补**重复边检测**。
- 验证：五连验证全绿（build 625 页/对比度 337 页）；提交 af227b9 已推送。
- 结论：补功能 + 修问题（体检盲区补全）。
- 新证据与经验：①第 93 轮体检验出重复边——第 9 项的又一盲区补上（重复 id/悬空边/重复边三类齐了）；②本轮状态文件补记 91-93 三轮（三轮完成时被流程打断未及时写入）——**记录与内容提交必须同步，攒批会丢**；③轮次记录块曾因候选池措辞相同被错位插入（64-69 块移到 90 后），已用脚本重组并校验连续。
- 下一轮入口：候选池——①mysql/connection-pool 第 3 篇候选（事务与连接交互/多数据源）或歇；②场景题 grep：抽奖变种/排行榜避让、找非同构；③ai 线：多模态 Agent 第 16 篇候选。每轮开工先同步+定界+体检基线。

### 第 94 轮（2026-09-08，第三十二次启动，内容补充模式）

- 选择：幻觉治理（ai/intermediate/llm 第 12 篇）——grep 确认幻觉仅 3 篇顺带引用无原理专篇；AI 面试 Top5 题，llm 分类质量维度的核心篇。
- 交付：`12-hallucination.md`（三层根源（训练目标只奖励"像"/知识边界/采样放大）、治理矩阵四层布防表、RAG 压低不消除的边界、自一致性与幻觉率检测）+ 侧边栏注册 + 图谱 halluc 节点与 1 条边。
- 验证：五连验证全绿（build 626 页）；提交 ead1afb 已推送（本篇纯表格）。
- 结论：补功能。
- 新证据与经验：①**状态文件与内容提交同步执行**（上轮 91-93 漏记的纠正）——内容 commit 后立即 Edit 状态文件并纳入下次提交；②astro.config.mjs 再遇 stale（并行会话修改中），重读重插纪律流畅化解。
- 下一轮入口：候选池——①场景题/ai 线继续；②linux 线歇。每轮开工先同步+定界+体检基线。

### 第 95 轮（2026-09-08，第三十三次启动，内容补充模式）

- 选择：时区处理（case-studies 第 23 篇）——grep 确认时区/UTC 4 篇分散（语言层）无架构层纪律篇；国际化后端必踩坑题。
- 交付：`23-timezone.md`（存储/传输/展示三层分离图、MySQL DATETIME vs TIMESTAMP 与 PG timestamptz、夏令时坑、差 8 小时四步排查清单）+ 侧边栏注册 + 图谱 tz 节点与 1 条边。
- 验证：五连验证全绿；提交 0d6fef4 已推送（本篇纯表格）。
- 结论：补功能。
- 下一轮入口：数据归档（grep 冷热分离仅 1 句确认）。

### 第 96 轮（2026-09-08，第三十三次启动）

- 选择：数据归档：冷热分离（case-studies 第 24 篇）——归档 18 篇分散、冷热分离仅设计框架篇一句；与 logrotate/Online DDL 互链。
- 交付：`24-archive.md`（在线表膨胀代价、冷热分离架构与查询路由、分批删除/先归档后删除/幂等、与分库分表正交、分区表关系）+ 侧边栏注册 + 图谱 archive 节点与 1 条边。
- 验证：五连验证全绿（build 628 页）；提交 58bd0b1 已推送（含时区篇 level 笔误修复——同款第三次，根因改善建议已入经验段）。
- 结论：补功能。
- 新证据与经验：①case-studies 系列到 24 篇；②**重复写入事故与修复**：第 95/96 轮记录被写入两份（第 95 轮收尾写一次、第 96 轮收尾"95/96 一起补"又写一次）——已用脚本去重重组；教训：**每轮收尾的记录写入前先 grep 确认该轮记录不存在**。
- 下一轮入口：候选池——①场景题 grep：数据订阅（CDC）/秒杀变种同构避；②ai 线：Agent 工具集设计篇候选；③linux 线歇。每轮开工先同步+定界+体检基线。

### 第 100 轮（2026-09-08，第三十五次启动，内容补充模式，第 100 轮里程碑）

- 选择：风控系统（case-studies 第 27 篇）——风控 10 篇分散提及（抽奖/秒杀/登录/评论各篇防刷一句带过）、设备指纹零覆盖；对抗性系统的体系化收口。
- 交付：`27-risk-control.md`（三层布防图（事前名单指纹/事中实时决策/事后离线反哺）、黑白灰名单+设备指纹+关联分析、规则引擎自研 vs Drools、误杀分级处置（拒/加验/放行观察/申诉））+ 侧边栏注册 + 图谱 riskctl 节点与 1 条边（lottery→riskctl"防刷的风控底座"）。
- 验证：五连验证全绿（build 632 页）；提交 ee02c9c 已推送（本篇纯表格）。
- 结论：补功能。
- 新证据与经验：①**第 100 轮里程碑**：场景题系列 27 篇 + AI 双线 30 篇 + 各方向补全，站点 632 页；②风控篇是"跨场景收口"第二例（对账篇第一例）——多个场景篇共用的子系统值得独立收口篇；③"对抗性系统的 KPI 是新攻击响应速度"——风控/安全类内容的统一视角。
- 下一轮入口：候选池——①场景题：秒杀变种同构避、全局搜索已写、限流已写——高频场景池见底，转 ai 线；②ai/intermediate/agent 第 16 篇候选（Agent 工具集设计：工具粒度/描述质量/错误返回）；③mongodb 聚合进阶（中频备选）。每轮开工先同步+定界+体检基线+PATH 前缀。

### 第 97 轮（2026-09-08，第三十四次启动，内容补充模式）

- 选择：数据订阅与 CDC（case-studies 第 25 篇）——grep 确认 CDC 场景专篇缺（Debezium/Canal 5 篇分散提及）；缓存失效/索引同步/数仓 ETL 的公共底座。
- 交付：`25-cdc.md`（轮询三宗罪、binlog 解析原理链路图、至少一次+下游幂等铁律、DDL 与全量增量切换、Canal vs Debezium 选型）+ 侧边栏注册 + 图谱 cdc 节点与 1 条边（"归档与订阅同源"←archive）。
- 验证：五连验证全绿（build 629 页/对比度 340 页）；提交 4ca8d53 已推送。
- 结论：补功能。
...
- 交付：`07-promo-readiness.md`（三阶段保障日历图、全链路压测与容量水位 70%、预案四要素（触发/动作/负责人/效果）、值守编排与升级路径、复盘时间线与行动项闭环）+ 侧边栏注册 + 图谱 promo 节点与 1 条边。
- 验证：五连验证全绿（build 630 页）；提交 8b75769 已推送（本篇纯表格）。
- 结论：补功能。
- 下一轮入口：候选池——①场景题/ai 线继续；②linux 线歇。每轮开工先同步+定界+体检基线+PATH 前缀。
