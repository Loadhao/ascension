...
```mermaid
flowchart TB
    C["协调节点<br/>from = 99000, size = 10"] -->|"每个分片都要 99010 条排序结果"| S1["分片 1"]
    C -->|"每个分片都要 99010 条排序结果"| S2["分片 2"]
    S1 -->|"传回 99010 条 doc_id"| C
    S2 -->|"传回 99010 条 doc_id"| C
    C -->|"全局排序后丢掉前 99000 条，只留 10 条<br/>翻得越深，每个分片白白传得越多"| OUT["返回 10 条"]
