...
| `id` | 节点唯一标识，边通过它引用 |
| `label` | 节点显示文本 |
| `href` | 可选；填写后节点加粗描边并标记为可点击，点击跳转到对应笔记 |
...
另有全站级的「[知识全景](/panorama/)」图谱：由 `src/lib/notes.ts` 在构建期
自动聚合全部方向与分类生成（`src/components/GlobalGraphIsland.astro`），
节点大小代表该方向/分类收录的知识点数量，无需手工维护数据。
