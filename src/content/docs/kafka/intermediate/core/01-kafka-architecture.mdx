...
segment 的一生（写入 → 滚动 → 读取 → 整段删除）可以按帧播放着看：

<AlgorithmVizIsland demo="kafka-segment" />

不想盯着屏幕时，下面这条 53 秒短片把这四步逐帧讲一遍（内容与上面的动画一帧一句对应）：

<AlgorithmVizIsland demo="kafka-segment-video" />
