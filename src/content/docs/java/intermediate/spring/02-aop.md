...
这也是 @Transactional 失效的头号原因（详见[事务篇](/java/intermediate/spring/04-transaction/)）。

## 切点表达式速查与切面排序

execution 是主力，分解一下各占位：`execution(* com.demo.service..*.save*(..))`
= 返回任意（*）· 包及子包（..）· save 前缀方法 · 参数任意（..）。其余
指示器按"匹配什么"记：

| 指示器 | 匹配对象 | 典型场景 |
|---|---|---|
| `execution(签名)` | 方法签名 | 通用日志、计时 |
| `@annotation(注解)` | **方法上**贴的注解 | 自定义注解 + 切面（如 @RateLimit） |
| `@within(注解)` | **类上**贴的注解 | 类级开关 |
| `within(包/类型)` | 类型或包 | 圈定范围 |
| `bean(名字)` | Bean 名称/通配 | 指定 Bean 生效 |

表达式之间支持 `&&`、`||`、`!` 组合；`@annotation` 是业务里最常见的
套路——声明一个注解当开关，切面拦所有贴它的方法，语义清晰且零入侵。

**多个切面谁在外层**：给切面类标 `@Order(数字)`，**值越小越靠外**，
前置通知先执行、后置通知最后执行。@Transactional 的切面默认排在最低
优先级（最内层），想让自己的切面包住事务（比如先记审计再开事务），
必须把 @Order 设得比它小；两个切面都不写 @Order 则顺序不确定——
"日志有时在事务外有时在事务内"这种灵异现象就是这么来的。
...
|---|---|
| 自调用 | this 不走代理（上文） |
| final 方法 | CGLIB 无法覆写，直接走原方法 |
| static / private 方法 | 代理基于方法覆写/接口，拦不到 |
| `new` 出来的对象 | 没经过容器，天生没有代理 |

## 小结

- AOP 抽横切逻辑；Spring AOP = 运行时动态代理，只支持方法级连接点。
- 注入的是代理对象；JDK 需要接口，CGLIB 生成子类，Boot 2.x 默认 CGLIB。
- 切点首选 execution 与 @annotation；多切面用 @Order 排座次，值越小越
  在外层，不标则顺序不确定。
