...
## 自我保护到底保护谁：触发阈值源码级解析（深入）

很多人把自我保护当"玄学"。其实它的触发是一条**可计算的阈值判断**，Eureka
的核心逻辑在 `AbstractInstanceRegistry` 的自我保护器里简化如下：

```text
期望续约数 = 上周期总续约数 × (1 - 容忍比例)     // 容忍比例默认 0.15
实际续约数 = 本次周期收到的所有 renewal
进入自我保护 ⇐  实际续约数 < 期望续约数
                 且 连续触发阈值的最小保护判定成立
```
...
1. **不是"实例挂了就保护"**，而是"续约总量断崖式下跌"才触发——偶发掉线
   不会，批量掉线（网络分区）才会。
2. **触发保护 = 停止剔除**，即"宁可留着可能是死实例的条目"，也别把表清空。
   初衷是**保护"活着的那一侧"**：网络分区时，分区两侧都要本地的服务列表
   可用，若无保护，某一侧会被剔空。
3. **生产该不该关（`enable-self-preservation=false`）**：要看你的网络是否
   真的稳定。日志区/云内网稳定 → 可以关，让下线真实反映；公网/多机房抖动
   → 建议留着，配合调用方重试。**别一刀切**。
...
| | Eureka | Nacos 临时实例 |
|---|---|---|
| 判定 | 客户端心跳续约 + 服务端剔除非续约 | 心跳间隔 + 服务端超时判定 |
| 默认剔除延迟 | 约 90s（3×30s 心跳） | 约 45s（15s 阈值 × 心跳） |
| 自我保护 | 开启（可关） | 有保护阈，默认弱 |

这两个数字决定了"实例挂了多久调用方仍可能路由到它"，是设计重试窗口的依据。
