...
| 机制                                                    | 解决的问题         | 核心一句话                      |
| ----------------------------------------------------- | ------------- | -------------------------- |
| [子智能体](/ai/intermediate/agent/09-subagent/)              | 上下文污染         | 大任务拆小，每个子任务干净的消息历史         |
| [任务系统](/ai/advanced/agent/02-task-system/)               | 计划不持久         | 文件持久化的任务图，可依赖、可认领、跨会话恢复    |
| [后台任务](/ai/advanced/agent/03-background-tasks/)          | 慢操作阻塞         | 慢操作丢后台线程，主循环继续推理           |
| [定时调度](/ai/advanced/agent/04-cron-scheduler/)            | 周期性靠人推        | 按时间表生产工作，调度与执行解耦           |
| [Agent 团队](/ai/advanced/agent/05-agent-teams/)           | 单 Agent 注意力不够 | 文件收件箱 + 常驻队友线程，并行推进        |
| [团队协议](/ai/advanced/agent/06-team-protocols/)            | 协作失控          | 显式消息契约：request\_id 关联请求与响应 |
| [自主智能体](/ai/advanced/agent/07-autonomous-agents/)        | 调度瓶颈          | 队友自己看板、自己认领，不等 Lead 派单     |
| [Worktree 隔离](/ai/advanced/agent/08-worktree-isolation/) | 文件冲突          | 并行 Agent 需要隔离的文件系统，如同隔离的对话 |
| [综合 Harness](/ai/advanced/agent/09-comprehensive-agent/) | 机制归位          | 机制很多，循环一个                  |
