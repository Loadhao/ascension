...
const DIRECTION_ORDER = ['linux', 'nginx', 'network', 'security', 'java', 'netty', 'js', 'typescript', 'react', 'vue', 'mysql', 'postgresql', 'redis', 'mongodb', 'elasticsearch', 'distributed', 'zookeeper', 'etcd', 'seata', 'middleware', 'kafka', 'rocketmq', 'mqtt', 'rabbitmq', 'python', 'algorithm', 'ai', 'langchain', 'spring-ai', 'docker', 'kubernetes', 'git', 'tools'];
...
export async function getSiteData(): Promise<SiteData> {
  ...
  for (const entry of entries) {
    ...
    // 方向首页：<dir>（index 规范化）
    if (segs.length === 1 && !RESERVED_ROOT_IDS.has(entry.id)) {
